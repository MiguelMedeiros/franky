...
export function DialogReportPost({ open, onOpenChange, postId }: DialogReportPostProps) {
  ...
  const renderContent = () => {
    switch (true) {
      ...
      case step === REPORT_POST_STEPS.REASON_INPUT && selectedIssueType !== null:
        return (
  ...
  return (
    <Atoms.Dialog open={open} onOpenChange={onOpenChange}>
      <Atoms.DialogContent className="w-2xl" hiddenTitle="Report Post" onOpenAutoFocus={(e) => e.preventDefault()}>
        <Atoms.DialogHeader>
