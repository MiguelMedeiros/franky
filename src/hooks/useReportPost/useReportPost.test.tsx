...
// Mock useCurrentUserProfile and useUserProfile hooks
...
vi.mock('@/hooks', async (importOriginal) => {
  ...
  return {
    ...
    useUserProfile: (pubky: string) => mockUseUserProfile(pubky),
  };
});
...
describe('useReportPost', () => {
  beforeEach(() => {
    ...
    // Default: author profile loaded
    mockUseUserProfile.mockReturnValue({
      profile: { name: TEST_AUTHOR_NAME },
      isLoading: false,
    });
  ...
  describe('selectIssueType', () => {
    ...
    it('should clear any existing error when selecting issue type', () => {
      const { result } = renderHook(() => useReportPost(TEST_POST_ID));

      // Simulate having an error
      act(() => {
        result.current.selectIssueType(REPORT_ISSUE_TYPES.HATE_SPEECH);
      });
  ...
  describe('goBackToIssueSelection', () => {
    it('should return to issue selection step and clear reason', () => {
      const { result } = renderHook(() => useReportPost(TEST_POST_ID));

      // Advance to reason step
      act(() => {
        result.current.selectIssueType(REPORT_ISSUE_TYPES.PERSONAL_INFO);
      });
      ...
      expect(result.current.step).toBe(REPORT_POST_STEPS.REASON_INPUT);
      expect(result.current.reason).toBe('Some reason');

      // Go back
      act(() => {
        result.current.goBackToIssueSelection();
      });
  ...
  describe('submit', () => {
    ...
    it('should not submit when currentUserPubky is null', async () => {
      ...
      expect(result.current.isSubmitting).toBe(false);
      expect(result.current.isSuccess).toBe(false);
      expect(result.current.error).toBe('User profile not loaded. Please try again.');
      expect(global.fetch).not.toHaveBeenCalled();
    });
